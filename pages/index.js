        ...
        <aside style={s.sidebar}>
          <div style={s.sidebarInner}>
            <div style={s.sidebarLogo}>
              ...
              <span style={s.logoText}>Strange Brain</span>
            </div>
            <nav style={s.nav}>
              <div style={s.navItem}>
            ...
            <div style={s.sidebarFooter}>
              <div style={s.footerLabel}>JIS X 8341-3 / WCAG 2.0<br />準拠ルール適用済み</div>
            </div>
          </div>
        </aside>
        ...
        <main style={s.main}>
          <div style={s.mainInner}>
            ...
            <div style={s.pageHeader}>
              <h1 style={s.pageTitle}>画像 alt テキスト生成</h1>
              <p style={s.pageDesc}>WordPressの公開済み記事をそのままコピー＆ペーストして、アクセシビリティとSEOに配慮したaltテキストを自動生成します。</p>
            </div>
            ...
            <div style={s.card}>
              ...
              <button onClick={generate} disabled={loading} style={{ ...s.genBtn, ...(loading ? s.genBtnLoading : {}) }}>
                {loading ? (
            ...
            {results.length > 0 && (
              <div>
                <div style={s.resultsHeader}>
                  ...
                  <span style={s.rulesBadge}>JIS / WCAG準拠</span>
                </div>
                {results.map((r, i) => {
                      ...
                      <div style={s.rcHeader}>
                        ...
                        <div style={s.rcMeta}>
                          ...
                          {(r.filename || img.filename) && (
                            <div style={s.rcFilename}>{r.filename || img.filename}</div>
                          )}
                        </div>
                      </div>
...
const s = {
  ...
  sidebar: { width: 220, background: '#fff', borderRight: '1px solid #e2e8f0', flexShrink: 0, position: 'fixed', top: 0, left: 0, height: '100vh', display: 'flex', flexDirection: 'column' },
  sidebarInner: { display: 'flex', flexDirection: 'column', height: '100%', padding: '1.5rem 1rem' },
  sidebarLogo: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: '2rem', paddingBottom: '1.25rem', borderBottom: '1px solid #e2e8f0' },
  logoMark: { width: 32, height: 32, borderRadius: 8, background: '#2b70ef', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, flexShrink: 0 },
  logoText: { fontSize: 14, fontWeight: 600, color: '#1e293b' },
  nav: { flex: 1 },
  navItem: { display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 8, background: '#eff6ff', color: '#2b70ef', fontSize: 13, fontWeight: 500 },
  sidebarFooter: { paddingTop: '1rem', borderTop: '1px solid #e2e8f0', marginTop: 'auto' },
  footerLabel: { fontSize: 11, color: '#64748b', lineHeight: 1.6 },
  main: { flex: 1, marginLeft: 220 },
  mainInner: { maxWidth: 800, margin: '0 auto', padding: '2.5rem 2rem 4rem' },
  pageHeader: { marginBottom: '1.75rem' },
  pageTitle: { fontSize: 24, fontWeight: 600, color: '#0f172a', letterSpacing: '-0.02em', marginBottom: 8 },
  pageDesc: { fontSize: 14, color: '#64748b', lineHeight: 1.65 },
  ...
  fieldLabel: { display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 8 },
  pasteArea: {
    width: '100%', minHeight: 320, maxHeight: 560, overflowY: 'auto',
    border: '1.5px dashed #cbd5e1', borderRadius: 10, padding: '16px 18px',
    fontSize: 14, color: '#1e293b', background: '#f8fafc', outline: 'none',
    lineHeight: 1.75, transition: 'border-color 0.15s, background 0.15s',
    cursor: 'text',
  },
  ...
  genBtn: { marginTop: 16, padding: '10px 24px', background: '#2b70ef', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 8, transition: 'background 0.15s' },
  ...
  rulesBadge: { fontSize: 11, color: '#0369a1', background: '#e0f2fe', border: '1px solid #bae6fd', borderRadius: 20, padding: '2px 8px', fontWeight: 500 },
  ...
  copyBtn: { padding: '8px 14px', fontSize: 12, fontWeight: 500, border: '1px solid #cbd5e1', borderRadius: 8, background: '#fff', color: '#475569', cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4, transition: 'all 0.1s' },
